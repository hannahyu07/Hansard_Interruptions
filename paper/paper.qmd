...
The model was implemented in R using the `rstanarm` package [@rstanarm], which applies Markov chain Monte Carlo (MCMC) sampling to estimate the posterior distributions of the model parameters. To optimize computational efficiency, I randomly sampled 1000 observations for model fitting, ensuring a balance between computational feasibility and statistical reliability.



## Model justification

A relationship is expected between gender, party affiliation, and speech interruptions in parliamentary debates. Male speakers are likely to face more interruptions than female speakers, reflecting common patterns in political discourse. However, interruptions may still occur for female speakers depending on the context of their speech.

For party affiliation, the ALP and LNP, as the two main parties, are expected to show different interruption patterns. ALP members may experience more interruptions, especially when in opposition, while LNP members may face fewer interruptions, particularly when in government. Other parties and independents may not follow a clear pattern due to their varying roles in debates.
